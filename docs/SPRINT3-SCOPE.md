...
**Ruflo benchmark protocol:** `benchmark-obrien-obrien-vs-ruflo.md`

---

## Bet

After Philipp approves slices, the pipeline executes, reviews, and recovers without Philipp until O'Brien delivers a demo or escalates a terminal problem.
...
| **O'Brien drain** | O'Brien is passive — Philipp has to nudge her on every error or escalation. Drain is a Cowork scheduled task (every 10 min) that checks `bridge/obrien-events.jsonl` for STUCK/ERROR/ALL_COMPLETE events and decides autonomously. | ADR accepted. Depends on Wormhole server shipping first. Context from OBRIEN.md via `/wrap-up` discipline. |
| **Nog** | Rename Anon → Nog. O'Brien writes the role spec from scratch. Adds: kanban history per slice (what was tried, assessment, delta to ACs), explicit `i < 6` counter, escalate to O'Brien at max. | O'Brien writes spec, O'Brien implements. Not an Anon diagnosis — a clean role definition. |
...
- `docs/architecture/OBRIEN-ACTIVATION-ADR.md` — O'Brien drain design
...
**Headless consent behavior:** Does consent granted in an interactive Cowork session carry to a headless scheduled task? Dax tests live during O'Brien activation wiring. If it doesn't, a consent preflight mechanism is built then. Decision deferred until test result is known — don't plan for it now.
