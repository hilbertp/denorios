...
test('J-devops-station slice-340-ac-5 — Pipeline A surfaces a low-confidence AC for the operator; ruling on it unlocks the merge', async ({ page }) => {
  await aheadState(page);
  // First the check is NOT ready — one AC needs a human call.
  const NEEDS_YOU = { ready: false, verdict: 'NEEDS_YOU',
    summary: { checked: 2, passed: 1, autoUpdate: 0, flagged: 1, kept: 0 },
    flagged: [{ tag: 'slice-340-ac-6', title: 'the merge-pressure pill reflects release risk with a rising/falling trend', reason: 'No test guards this AC. Worth a permanent guard?' }],
  ...
  await page.route('**/api/check-test-updates', r =>
    r.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(NEEDS_YOU) }));
  // Recording the ruling resolves it.
  let decided = false;
  await page.route('**/api/check-test-updates/decide', r => {
    decided = true;
    r.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(RESOLVED) });
  });

  await page.goto('/');
  await page.locator('#check-updates-btn').click();

  // The decision overlay surfaces the flagged AC with the two rulings.
  const overlay = page.locator('#test-updates-overlay');
  await expect(overlay).toBeVisible();
  await expect(overlay).toContainText('slice-340-ac-6');
  await expect(overlay).toContainText(/Update the test to match/i);
  await expect(overlay).toContainText(/Keep as is/i);

  // Rule on it → the gate unlocks.
  await overlay.getByRole('button', { name: /Update the test to match/i }).click();
...
test('J-devops-station slice-340-ac-6 — the Merge Pressure pill shows a release-risk band (rising at 43%)', async ({ page }) => {
  ...
  // The pill is now a semicircular gauge: "43% · merge pressure · rising".
  const gauge = page.locator('.dvs-gauge');
