...
function buildNogPrompt({ id, round, sliceFileContents, doneReportContents, gitDiff, scopeDiff, slicePath, lane }) {
  ...
  return [
    ...
    '',
    '### Part 1: Code Review',
    'Review the code changes per ROLE.md: correctness, quality, lint, test coverage.',
    "Regressions are caught by machines, not by you: GitHub runs the safety-net suite when the slice lands on dev, Julian's stage runs both suites, and the Promote button runs them again.",
    'Do not run the full safety-net suite (`npm test`, `node --test regression/`), the browser suite (`npx playwright test`), or the lock builders (`build-coverage-map`, `build-ac-manifest`). You may run the test files this slice adds or changes, and the linter on the changed files.',
    '',
    ...
    '',
    '   The whole file looks exactly like this, and nothing else:',
    '',
    '---',
    'verdict: ACCEPTED',
    'summary: "One line."',
    '---',
    '',
    '   The first line of the file is `---` and the frontmatter ends with a line that is exactly `---`. Write nothing before the first `---`.',
    '',
