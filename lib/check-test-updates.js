...
//   SURFACE          → high · pass         (slice 390: a surface-lane AC with no guard. The
//                                           lane already answered the question MISSING asks,
//                                           so asking it again is the dead end of 2026-09-01
//                                           — a row the operator cannot honestly decide,
//                                           blocking a merge that was never in doubt)
...
// What the operator reads beside a surface-lane AC — the evidence that already exists,
// named, so the row explains itself rather than looking like a check that was skipped.
const SURFACE_REASON = 'Surface change: Jordan\u2019s review is the evidence; the browser suite covers the screen at the gate.';
...
function triageTag(tag, status, manifestEntry, decision) {
  ...
  switch (status) {
    ...
    case 'SURFACE':
      // 'pass', not a fourth action: the overlay lists `flagged` and counts the rest as
      // "already covered or handled automatically", so a surface AC is accounted for on
      // screen without a card and without a markup change.
      return { tag, title, status, confidence: CONF.HIGH, action: 'pass', needsHuman: false,
...
module.exports = { triage, triageTag, CONF, SURFACE_REASON };
