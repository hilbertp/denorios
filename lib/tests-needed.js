...
//           surfaceOnlyFiles:['<path>'] }
// NB the OUTPUT field of the same name holds behaviourFile OBJECTS, not paths: this one is
// the gate's question ("which paths did only surface commits touch?"), that one its answer.
// decide()'s result is not a facts object and must not be fed back in as one.
function decide(facts) {
  ...
  // Slice 390: a file every one of whose commits declared `Lane: surface` is not policed for
  // a missing guard. gather() decides WHICH files those are (it alone can see the commits);
  // this stays pure and only honours the list. One core or unlabelled commit touching the
  // file — or no attributed commit at all — and it is policed exactly as before.
  const surfaceOnly = new Set(facts.surfaceOnlyFiles || []);
  const policed = (bf) => !isGateSpec(bf.path) && !surfaceOnly.has(bf.path) && !notNeeded(bf.path) && !corroborated(bf.path, checks, covMap);
  const newBehaviourNoTest = behaviourFiles.filter(bf => bf.isNew && policed(bf));
  const unguardedSourceChanges = behaviourFiles.filter(bf => policed(bf));
  const surfaceOnlyFiles = behaviourFiles.filter(bf => surfaceOnly.has(bf.path));
  ...
  for (const tag of Object.keys(headTags)) {
    ...
    if (!b) continue;
    const hashMoved = !!(b.acHash && h.acHash && b.acHash !== h.acHash);
    // Slice 390 closes the door the lane opens: with identical text, re-declaring a core
    // criterion as surface turns a MISSING into a SURFACE — a criterion that owed a test
    // no longer owing one, with nothing in the diff to show for it. That is a spec edit,
    // and it needs the spec owner's signature like any other (AC-Change-OK + Spec-Owner).
    // A base entry predating this slice carries no lane at all; absent reads core.
    const laneFlipped = (b.lane || 'core') === 'core' && h.lane === 'surface';
    if (hashMoved || laneFlipped) acMutated.push(tag);
  }
  ...
  return {
    ...
    newBehaviourNoTest, unguardedSourceChanges, surfaceOnlyFiles, overridden, mismatchedOverride,
...
function gather({ base, head, repoRoot }) {
  ...
  const surfaceOnlyFiles = surfaceOnlyChangedFiles({ base, head, repoRoot });
  let covMap = null;
  ...
  return { behaviourFiles, checks, trailers, surfaceOnlyFiles, covMap, coverage, acManifest, acEnforce };
}

// ── Lane attribution (slice 390, ADR-PROOF-LANES) ───────────────────────────
//
// The paths in base..head whose EVERY touching commit declared `Lane: surface`. Those are
// changes to what the screen shows or says: Jordan's review is their evidence and the
// browser suite covers the screen at the gate, so decide() does not ask them for a guard.
//
// THE FORMAT IS THE WHOLE POINT. `--format=%B%x00 --name-only` looks like the obvious way to
// read bodies and files together and is a trap: git emits a commit's file list AFTER its
// format string, so every file lands in the NEXT `\0`-split record — attributed, silently,
// to the commit before it. Read the wrong way round, one surface commit would excuse the
// files of the core commit that follows it. `%x00%H%x01%B%x02` fences each record's sha,
// body and file list inside that record, so a file can only ever be read against its own
// commit's lane. A separate call from the trailer log above, deliberately: parseTrailers
// carries every override the gate honours, and it keeps reading the exact bytes it did before.
//
// Fails CLOSED in every direction. The log throws → no file is surface-only and everything is
// policed as it was yesterday. A merge commit lists no files → the files it merged have no
// attribution from it, and a file with no attributed commit at all is policed. A commit with
// no `Lane:` trailer reads core, which is the entire back catalogue.
function surfaceOnlyChangedFiles({ base, head, repoRoot }) {
  let raw = '';
  try { raw = git(['log', '--format=%x00%H%x01%B%x02', '--name-only', `${base}..${head}`], repoRoot); } catch (_) { return []; }

  const lanesByFile = new Map();
  for (const rec of raw.split('\x00')) {
    const sha = rec.indexOf('\x01');
    if (sha === -1) continue;                     // not a record (the empty head of the split)
    const rest = rec.slice(sha + 1);
    const end = rest.indexOf('\x02');
    if (end === -1) continue;                     // a truncated record attributes nothing
    // `|| 'core'` is this consumer's boundary default: laneOfCommitBody reports an undeclared
    // lane as null so the manifest deriver can fall back to a slice file's frontmatter, and a
    // commit log has no slice file to fall back to. Unlabelled = core = policed, as before.
    const lane = laneOfCommitBody(rest.slice(0, end)) || 'core';
    for (const line of rest.slice(end + 1).split('\n')) {
      const file = line.trim();
      if (!file) continue;
      if (!lanesByFile.has(file)) lanesByFile.set(file, []);
      lanesByFile.get(file).push(lane);
    }
  }

  const out = [];
  for (const [file, lanes] of lanesByFile) {
    if (lanes.length && lanes.every(l => l === 'surface')) out.push(file);
  }
  return out;
}
...
module.exports = { classify, decide, gather, surfaceOnlyChangedFiles, bucketOf, areaOf, parseTrailers, transitionFor, globToRe, corroborated, guardCountOf };
