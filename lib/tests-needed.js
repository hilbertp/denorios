...
// ── PURE verdict ────────────────────────────────────────────────────────────
// facts = { behaviourFiles:[{path,area,isNew}], checks:[{file,tag,area,kind,direction,rename}],
...
function gather({ base, head, repoRoot }) {
  ...
  for (const tf of testFilesChanged) {
    ...
    for (const k of Object.keys(byTag)) {
      ...
      checks.push({ file: tf, tag: k, area: areaOf(tf), kind, direction: e.direction, rename: e.rename || null });
    }
  }
  let bodies = [];
